...
# torch.save(netD.state_dict(), 'discriminator4.pth')
...
for epoch in range(num_epochs):
    ...
    for i, data in progress_bar:
        ...
        real_cpu = inputs
        b_size = real_cpu.size(0)
        label = torch.zeros(b_size, num_classes)  # Create a tensor of zeros
        label.scatter_(1, labels.view(-1, 1), 1)  # One-hot encode the labels
        label = label.to(device)  # Move the tensor to the MPS device
        # Forward pass real batch through D
        output = netD(real_cpu).view(-1, num_classes)
        # Calculate loss on all-real batch
        errD_real = criterion(output, label)
        # Calculate gradients for D in backward pass
        errD_real.backward()
        ...
        label.fill_(0)
        # Classify all fake batch with D
        output = netD(fake.detach()).view(-1, num_classes)
        # Calculate D's loss on the all-fake batch
        errD_fake = criterion(output, label)
        # Calculate the gradients for this batch, accumulated (summed) with previous gradients
        errD_fake.backward()
        ...
        label.fill_(1)  # fake labels are real for generator cost
        # Since we just updated D, perform another forward pass of all-fake batch through D
        output = netD(fake).view(-1, num_classes)
        # Calculate G's loss based on this output
        errG = criterion(output, label)
        # Calculate gradients for G
        errG.backward()
